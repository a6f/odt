...
impl<'a> NodeChange<'a> {
    ...
    pub fn exists(&self) -> bool {
        matches!(self, NodeChange::TopNode(_) | NodeChange::ChildNode(_))
    }
}
...
impl<'a> PropChange<'a> {
    ...
    pub fn exists(&self) -> bool {
        matches!(self, PropChange::Prop(_))
    }
}
...
pub fn merge4<'i>(
    ...
    let (root, node_labels) = merge_with_events(dts, scribe, &mut |e| match e {
        MergeEvent::AddNode { path, create } => {
            let chg = match create {
                NodeCreate::TopNode(x) => NodeChange::TopNode(x),
                NodeCreate::ChildNode(x) => NodeChange::ChildNode(x),
            };
            node_changes.entry(path.clone()).or_default().push(chg);
        }
        MergeEvent::DelNode { path, delete } => {
            // Record this node and all extant descendants as deleted.
            let end = path.descendants_end();
            let node_bounds = (Bound::Included(path), end.as_ref());
            let node_change = match delete {
                NodeDelete::TopDelNode(x) => NodeChange::TopDelNode(x),
                NodeDelete::DelNode(x) => NodeChange::DelNode(x),
            };
            for (_, v) in node_changes.range_mut(node_bounds) {
                if v.last().unwrap().exists() {
                    v.push(node_change);
                }
            }
            // Record all extant contained properties as deleted.
            // Skip exact matches, because a sibling property could have the same name as the node.
            let prop_bounds = (Bound::Excluded(path), end.as_ref());
            let prop_change = match delete {
                NodeDelete::TopDelNode(x) => PropChange::TopDelNode(x),
                NodeDelete::DelNode(x) => PropChange::DelNode(x),
            };
            for (_, v) in prop_changes.range_mut(prop_bounds) {
                if v.last().unwrap().exists() {
                    v.push(prop_change);
                }
            }
        ...
        MergeEvent::AddProp { path, name, prop } => {
            let chg = PropChange::Prop(prop);
            prop_changes.entry(path.join(name)).or_default().push(chg);
        }
        MergeEvent::DelProp {
            path,
            name,
            ...
            let chg = PropChange::DelProp(delprop);
            prop_changes.entry(path.join(name)).or_default().push(chg);
        }
        MergeEvent::AddLabel { .. } => {}
    });
